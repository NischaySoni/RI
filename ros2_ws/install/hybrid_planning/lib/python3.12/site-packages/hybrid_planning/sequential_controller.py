...
class Replanner(Node):
    def __init__(self):
        # The node name is changed to be unique
        super().__init__('sequential_controller_node')
        self.get_logger().info('Sequential Controller node is active.')
...
async def main(args=None):
    ...
    # The class name is still 'Replanner', but the node name is different
    sequential_node = Replanner() 
    
    # 1. Set the initial goal
    sequential_node.set_goal()
    
    # 2. Run the first (and only) attempt
    # This will fail because monitor_motion.py will find the collision
    success = await sequential_node.get_and_run_plan()
    
    # --- REPLANNING LOGIC IS REMOVED ---
    # The 'if not success:' block is intentionally deleted for this sequential run.
    
    if success:
        sequential_node.get_logger().info("Mission Complete!")
    else:
        sequential_node.get_logger().error("Mission Failed.")
        
    sequential_node.destroy_node()
    rclpy.shutdown()
